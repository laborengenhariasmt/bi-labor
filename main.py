...
# URLs - CERTIFIQUE-SE QUE SÃO OS LINKS DE "PUBLICAR NA WEB" -> "CSV"
...
def load_data(url):
    try:
        # O segredo para o 401 às vezes é o cabeçalho de requisição
...
# CARGA ÚNICA (Removi a duplicidade que estava no seu código)
...
if not df_propostas_raw.empty:
    ...
    with aba_comissoes:
        if not df_com_raw.empty:
            ...
            if col_val_rec and col_data_rec:
                ...
                if meses_com:
                    mes_sel = st.selectbox("Mês do Recebimento", meses_com, key="com_sel")
    ...
    st.error("Falha ao carregar Propostas. Verifique o link e as permissões de acesso do Google Sheets.")
